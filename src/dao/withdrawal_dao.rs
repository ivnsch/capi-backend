...
impl WithdrawalDao for WithdrawalDaoImpl {
    async fn init(&self) -> Result<()> {
        // TODO foreign key slot_id -> 1:n slots table
        let _ = self
            .client
            .execute(
    ...
    async fn save_withdrawal_request(
        ...
        let id_rows = self.client
            .query(
                "INSERT INTO withdrawal_request (project_id, slot_id, amount, description, date, complete) VALUES ($1, $2, $3, $4, $5, $6) RETURNING id;",
                &[
                    &project_id,
    ...
    async fn load_withdrawal_requests(
        ...
        let project_rows = self.client.query(
            "SELECT id, project_id, slot_id, amount, description, date, complete FROM withdrawal_request WHERE project_id=$1 ORDER BY date DESC;",
            &[&project_id]).await?;
...
mod test {
    ...
    async fn test_insert_and_load_a_withdrawal() -> Result<()> {
        ...
        let request = WithdrawalRequest {
            ...
            slot_id: "1".to_owned(), // currently arbitrary
